...
test.skip('parseToolArguments parses object json', () => {
  expect(ParseToolArguments.parseToolArguments('{"uri":"file:///x"}')).toEqual({ uri: 'file:///x' })
})

test.skip('parseToolArguments returns empty object for non-string input', () => {
  expect(ParseToolArguments.parseToolArguments(42)).toEqual({})
})

test.skip('parseToolArguments returns empty object for invalid json', () => {
  expect(ParseToolArguments.parseToolArguments('{')).toEqual({})
})

test.skip('parseToolArguments returns parsed array for array json', () => {
